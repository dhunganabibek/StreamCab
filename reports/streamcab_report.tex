...
The data download utility supports monthly yellow taxi parquet files from 2015 to 2025, with a default pull range of \texttt{2022-01} through \texttt{2024-12}. For the quick-start workflow documented in the repository, a smaller slice (for example \texttt{2022-01} to \texttt{2022-03}) is used for rapid iteration and reproducible local runs.
...
\bottomrule
\end{tabular}
\caption{Data dictionary across ingestion, aggregation, and prediction stages.}
...
\bottomrule
\end{tabular}
\caption{Primary quality rules used before window aggregation.}
\label{tab:quality_rules}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{pu\_location\_id}, \texttt{hour}, \texttt{day\_of\_week}, \texttt{is\_weekend}
    \item \texttt{avg\_trip\_distance}, \texttt{avg\_duration\_min}, \texttt{avg\_speed\_mph}, \texttt{trip\_count}
\end{itemize}

The XGBoost regressor is trained with \texttt{max\_depth=5}, \texttt{learning\_rate=0.05}, \texttt{subsample=0.9}, \texttt{colsample\_bytree=0.9}, \texttt{tree\_method=hist}, and \texttt{random\_state=42}. Training uses a temporal 80/20 train-test split, and the training partition is further divided into core-train plus validation for continued batch-wise boosting.
...
\bottomrule
\end{tabular}
\caption{Configuration snapshot for reproducible local runs.}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Run one-shot training to generate \texttt{traffic\_model.joblib} and \texttt{metrics.json}.
...
\bottomrule
\end{tabular}
\caption{Operational SLOs used for local reliability checks.}
\label{tab:slo_targets}
\end{table}
